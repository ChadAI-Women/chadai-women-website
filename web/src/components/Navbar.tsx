      ...
      <div className="section-container">
        ...
        {isOpen && (
          <div
            ...
            className="lg:hidden fixed top-[72px] left-0 right-0 z-50 h-[calc(100vh-72px)] bg-background border-t border-primary/10 shadow-elevated p-6 overflow-y-auto"
          >
            <ul className="flex flex-col gap-2">
              {navItems.map((item) =>
                item.children ? (
                  <li key={item.label} className="border-b border-primary/10 pb-2">
                    <div className="text-[11px] uppercase tracking-widest text-secondary-ink font-bold py-2">
                      {item.label}
                    </div>
                    {item.children.map((child) => (
                      <NavLink
