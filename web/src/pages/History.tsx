...
const PersonCard = ({ p }: { p: Person }) => (
  <div className="bg-background rounded-xl p-4 border border-border">
    ...
    <div className="text-sm text-secondary-ink font-semibold">{p.role}</div>
    {p.period && (
      ...
      <section className="py-20 bg-card">
        <div className="section-container max-w-4xl space-y-14">
          <Section icon={Users} title="Cofondatrices">
            {cofondatrices.length === 0 ? (
              <p className="text-muted-foreground italic">
                Les cofondatrices seront ajoutees apres validation officielle
                des informations.
              </p>
            ) : (
              ...
              <ol className="relative border-l-2 border-secondary/30 space-y-6 pl-6">
                {realisations.map((r, i) => (
                  <li key={i}>
                    ...
                    <div className="text-xs font-semibold text-secondary-ink uppercase tracking-wider">
                      {r.year}
                    </div>
